...
uint8_t UCB1_I2CMaster_GotNack(void) { return I2C_Drapeau_NACK; }

// indique si la lecture est termin閑
uint8_t UCB1_I2CMaster_Previous_ReadBytes_Finished(void) { return I2C_Drapeau_STOP; }

// indique si la lecture index閑 est termin閑
uint8_t UCB1_I2CMaster_Previous_ReadBytesIndexed_Finished(void) { return (RX_Pos >= RX_Nb); }

// indique si l'閏riture est termin閑
uint8_t UCB1_I2CMaster_Previous_WriteBytes_Finished(void) { return I2C_Drapeau_STOP; }

// indique si l'閏riture index閑 est termin閑
uint8_t UCB1_I2CMaster_Previous_WriteBytesIndexed_Finished(void) { return I2C_Drapeau_STOP; }
...
uint8_t UCB1_I2CMaster_ReadBytes(uint8_t Count)
{
    ...
    {
        while(EUSCI_B1->CTLW0 & EUSCI_B_CTLW0_TXSTT); // scrutation de l'envoi du START
        EUSCI_B1->CTLW0 |= EUSCI_B_CTLW0_TXSTP;       // envoi du STOP pour arr阾er la lecture
    }
    return 0;
}
...
uint8_t UCB1_I2CMaster_ReadBytes_Indexed(uint8_t RegAddress, uint8_t Count)
{
    ...
    while ((EUSCI_B1->STATW&EUSCI_B_STATW_BBUSY));    // V閞ification que le bus est disponible
    EUSCI_B1->IFG &= ~(EUSCI_B_IFG_RXIFG0 | EUSCI_B_IFG_TXIFG0 );
    EUSCI_B1->CTLW0 |= EUSCI_B_CTLW0_TR | EUSCI_B_CTLW0_TXSTT;  // lancement comm. en TX
    return 0;
}
...
void UCB1_I2CMaster_WriteBytes_Local(uint8_t Count)
{
    I2C_Drapeau_STOP = I2C_Drapeau_NACK = 0;          // mise � z閞o des drapeaux
    ...
    while ((EUSCI_B1->STATW&EUSCI_B_STATW_BBUSY));    // V閞ification  bus disponible
    EUSCI_B1->IFG &= ~(EUSCI_B_IFG_RXIFG0 | EUSCI_B_IFG_TXIFG0);
    EUSCI_B1->CTLW0 |= (EUSCI_B_CTLW0_TR + EUSCI_B_CTLW0_TXSTT);  // lancement comm. (TX+START)
}
...
uint8_t UCB1_I2CMaster_WriteBytes(uint8_t Count, uint8_t *Data)
{
	...
	UCB1_I2CMaster_WriteBytes_Local(Count);           // fonction locale d'envoi des donn閑s
	return 0;
}
...
uint8_t UCB1_I2CMaster_WriteBytesIndexed(uint8_t RegAddress, uint8_t Count, uint8_t *Data)
{
	...
	UCB1_I2CMaster_WriteBytes_Local(1 + Count);       // fonction locale d'envoi des donn閑s
    return 0;
}

void EUSCIB1_IRQHandler(void)
{
    if (EUSCI_B1->IFG & EUSCI_B_IFG_NACKIFG)          // drapeau NACKIFG : d閒aut de communication
    {
        EUSCI_B1->IFG &= ~EUSCI_B_IFG_NACKIFG;
        I2C_Drapeau_NACK = 1;                         // On indique la r閏eption d'un NACK
        EUSCI_B1->CTLW0 |= EUSCI_B_CTLW0_TXSTP;       // On stoppe la communication
    }

    if (EUSCI_B1->IFG & EUSCI_B_IFG_RXIFG0)           // une donn閑 a 閠� re鐄e
    {
		EUSCI_B1->IFG &= ~EUSCI_B_IFG_RXIFG0;         // on efface le drapeau de r閏eption
        RX_Data[RX_Pos] = EUSCI_B1->RXBUF;            // lire RXBUF (met RXIFG0 � 0)
        RX_Pos += 1;
    ...
    if (EUSCI_B1->IFG & EUSCI_B_IFG_TXIFG0)           // buffer d'envoi disponible
    {
        EUSCI_B1->IFG &= ~EUSCI_B_IFG_TXIFG0;
        if (TX_Pos < TX_Nb)                           // si des donn閑s doivent 阾re envoy閑s
        {
            EUSCI_B1->TXBUF = TX_Data[TX_Pos];        // 閏rire dans TXBUF (met TXIFG0 � 0)
            TX_Pos += 1;
        }
        else                                          // tout a 閠� envoy�
        {
            if (I2C_ReStart == 0)
            {
				EUSCI_B1->CTLW0 |= EUSCI_B_CTLW0_TXSTP; // on envoie le stop
			}
			else
            {
                EUSCI_B1->CTLW0 &= ~(EUSCI_B_CTLW0_TR); // Le maitre bascule en RX
                EUSCI_B1->CTLW0 |= EUSCI_B_CTLW0_TXSTT; // envoi du START
                if(RX_Nb == 1)                          // S'il y a un seul octet � recevoir
                {
                    while(EUSCI_B1->CTLW0 & EUSCI_B_CTLW0_TXSTT); // on attend la fin du START
                    EUSCI_B1->CTLW0 |= EUSCI_B_CTLW0_TXSTP;       // on envoie le STOP
                }
            }
        }
    ...
    if (EUSCI_B1->IFG & EUSCI_B_IFG_STPIFG)			  // drapeau STOP est activ�
    {
